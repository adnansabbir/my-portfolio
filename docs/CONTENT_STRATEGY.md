...
## Positioning
Software Engineer with 7+ years across full-stack and backend systems, currently
...
> Software Engineer with 7+ years building backend systems, distributed
...
- Full-stack/backend software engineer at Odoo R&D.
...
page automatically; identity fields from `data/site.ts`, `sameAs` pulled from
`data/social.ts`'s active links).
...
together. For a fixed page, add the title to `pageTitles` in `data/site.ts`
(via `withBrand()`) rather than writing the string inline: it embeds the
owner's name, and `data/site.ts` is the one place that's allowed to.
Per-post titles stay bare and inline — see "Page `<title>` format" in
`docs/CONVENTIONS.md`. The sitemap and Person schema need no manual touch (automatic /
